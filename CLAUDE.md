...
- Prompt dịch là **của người dùng** (v0.7). App không tự chèn khối ngữ cảnh nào nữa, chỉ thay `{{...}}` rồi nối đúng output contract ở cuối — parser dựa vào thẻ nên chỗ đó không giao ra. Thêm khối mới thì thêm biến trong `promptVars.ts`, đừng nối chuỗi trong `llm.ts`. Luồng tóm tắt section và ngữ cảnh chung vẫn giữ cách nối cũ.
- Biến rỗng thì `fillPrompt` **bỏ cả đoạn văn** chứa nó, nên thẻ và câu hướng dẫn phải nằm cùng đoạn với biến trong prompt mặc định.
...
- `POST /api/jobs/:id/reset` chỉ xoá thứ do các lần chạy sinh ra. Không đụng `source`, `source_override`, bố cục chunk, danh sách section và `jobs.context`. Chunk `skipped` giữ nguyên trạng thái, đừng gộp nó vào lệnh update chung.
...
| `src/lib/contextWindow.ts` | CR v0.7 — chọn nội dung cho khối ngữ cảnh mạch (tóm tắt các đoạn xa + nguyên văn N đoạn gần nhất). Trả **nội dung trần**, thẻ bao ngoài do prompt người dùng viết |
| `src/lib/promptVars.ts` | CR v0.7 — bảng biến `{{...}}` của prompt dịch + luật điền. Route dùng để điền, Settings dùng để hiện help text và cảnh báo thiếu biến |
